...
void gpu_ffnn(float *d_X, float *d_W1, float *d_W2, float *d_Y, float *d_Z, int N, int B, int BLOCK_SIZE, int STREAMS, cudaStream_t streams[])
{
    int subB = B / STREAMS;
    ...
    for (int s = 0; s < STREAMS; s++) {
        ...
        dim3 grid((N + BLOCK_SIZE - 1) / BLOCK_SIZE,
                    (subB + BLOCK_SIZE - 1) / BLOCK_SIZE);

        matmul_kernel<<<grid, block, 0, streams[s]>>>(d_X + offset, d_W1, d_Y + offset, N, subB);

        int relu_size = subB * N;
        relu_kernel<<<(relu_size + 255) / 256, 256, 0, streams[s]>>>(d_Y + offset, relu_size);

        matmul_kernel<<<grid, block, 0, streams[s]>>>(d_Y + offset, d_W2, d_Z + offset, N, subB);
    }
}
...
int main()
{
    ...
    gpu_ffnn(d_X, d_W1, d_W2, d_Y, d_Z, N, B, BLOCK_SIZE, STREAMS, streams);
